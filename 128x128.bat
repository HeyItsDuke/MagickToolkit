...
for /R "%input_folder%" %%f in (*.png) do (
    ...
    if !dry_run! == 0 (
        %imagemagick_path% "%%f" -resize 128x128 "%output_folder%\!relative_dir!%%~nxf"
    ) else (
        echo [Dry Run] Would resize PNG: "%%f" to 128x128 and save to "%output_folder%\!relative_dir!%%~nxf"
    )
)
...
for /R "%input_folder%" %%f in (*.*) do (
    ...
    if /I not "%%~xf"==".png" (
        ...
        if !dry_run! == 0 (
            %imagemagick_path% "%%f" -resize 128x128 "%output_folder%\!relative_dir!%%~nf.png"
        ) else (
            echo [Dry Run] Would convert and resize file: "%%f" to PNG and resize to 128x128, saving to "%output_folder%\!relative_dir!%%~nf.png"
        )
    )
)
