...
for /R "%input_folder%" %%f in (*.png) do (
    ...
    if !dry_run! == 0 (
        %imagemagick_path% "%%f" -resize 256x256 "%output_folder%\!relative_dir!%%~nxf"
    ) else (
        echo [Dry Run] Would resize PNG: "%%f" to 256x256 and save to "%output_folder%\!relative_dir!%%~nxf"
    )
)
...
for /R "%input_folder%" %%f in (*.*) do (
    ...
    if /I not "%%~xf"==".png" (
        ...
        if !dry_run! == 0 (
            %imagemagick_path% "%%f" -resize 256x256 "%output_folder%\!relative_dir!%%~nf.png"
        ) else (
            echo [Dry Run] Would convert and resize file: "%%f" to PNG and resize to 256x256, saving to "%output_folder%\!relative_dir!%%~nf.png"
        )
    )
)
