...
for /R "%input_folder%" %%f in (*.png) do (
    ...
    if !dry_run! == 0 (
        ...
        echo [Dry Run] Would resize PNG: "%%f" to 512x512 and save to "%output_folder%\!relative_dir!%%~nxf"
    )
)
...
for /R "%input_folder%" %%f in (*.*) do (
    ...
    if /I not "%%~xf"==".png" (
        ...
        if !dry_run! == 0 (
            ...
            echo [Dry Run] Would convert and resize file: "%%f" to PNG and resize to 512x512, saving to "%output_folder%\!relative_dir!%%~nf.png"
        )
    )
)
