...
set "target_resolution=convert_webp_lossy"
...
for /R "%input_folder%" %%f in (*.*) do (
	...
	rem Convert the non-PNG file to lossy WebP
...
rem Create the converted folder only if non-WebP files were found
if !non_png_found! == 1 mkdir "%output_folder%\converted" 2>nul

echo Batch conversion to lossy WebP completed!
pause
